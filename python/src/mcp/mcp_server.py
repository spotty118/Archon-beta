...
async def lifespan(server: FastMCP) -> AsyncIterator[ArchonContext]:
    ...
    with _initialization_lock:
        ...
        try:
            ...
            # Initialize enhanced MCP HTTP client with connection pooling
            logger.info("⚡ Initializing enhanced MCP HTTP client...")
            from src.server.services.mcp_http_client import initialize_mcp_http_client
            from src.server.config.service_discovery import get_api_url, get_agents_url
            
            api_url = get_api_url()
            agents_url = get_agents_url()
            mcp_client_ready = await initialize_mcp_http_client(api_url, agents_url)
            
            if mcp_client_ready:
                logger.info("✓ Enhanced MCP HTTP client initialized with connection pooling")
            else:
                logger.warning("⚠️ Enhanced MCP HTTP client initialization failed, falling back to individual connections")
            ...
            # Cleanup enhanced MCP HTTP client
            try:
                from src.server.services.mcp_http_client import cleanup_mcp_http_client
                await cleanup_mcp_http_client()
                logger.info("✓ Enhanced MCP HTTP client cleaned up")
            except Exception as e:
                logger.warning(f"⚠️ Error cleaning up MCP HTTP client: {e}")
