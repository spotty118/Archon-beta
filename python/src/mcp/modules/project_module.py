...
# Import enhanced MCP HTTP client with connection pooling
    ...
    async def manage_project(
        ...
        try:
            ...
            if action == "create":
                ...
                # Call Server API to create project using enhanced HTTP client
                result = await http_client.mcp_request(
                    method="POST",
                    service="api",
                    endpoint="/api/projects",
                    request_type=MCPRequestType.PROJECT_MANAGEMENT,
                    json_data={"title": title, "prd": prd, "github_repo": github_repo}
                )
                ...
                if result:
                    ...
                    return json.dumps({
                        "success": False, 
                        "error": "Failed to create project - no response from API service"
                    })
                ...
                if result:
                    return json.dumps({"success": True, "projects": result})
                else:
                ...
                if result:
                    ...
                    return json.dumps({
                        "success": False,
                        "error": f"Project {project_id} not found or failed to get project"
                    })
    ...
    async def manage_task(
        ...
        try:
            ...
            if action == "create":
                ...
                # Call Server API to create task using enhanced HTTP client
                result = await http_client.mcp_request(
                    method="POST",
                    service="api",
                    endpoint="/api/tasks",
                ...
                endpoint = "/api/tasks"  # Default endpoint
                params = {
                    "page": page,
                    "per_page": per_page,
                ...
                if filter_by == "project" and filter_value:
                    ...
                    endpoint = f"/api/projects/{filter_value}/tasks"
                ...
                # Make the API call using enhanced HTTP client
                result = await http_client.mcp_request(
                    method="GET",
                    service="api",
                    endpoint=endpoint,
                ...
                if result:
                    ...
                    return json.dumps({
                        "success": False, 
                        "error": f"Task {task_id} not found or failed to get task"
                    })
