...
async def crawl_knowledge_item(request: KnowledgeItemRequest):
    ...
    from ..config.security_config import validate_url
    ...
    try:
        ...
        safe_logfire_info(f"URL validation failed | url={str(request.url)} | error={e.detail}")
        raise
...
async def upload_document(
    ...
    try:
        # Import security validation
        from ..config.security_config import validate_file_upload
        ...
        try:
            ...
            safe_logfire_info(
                f"File upload validation failed | filename={file.filename} | error={e.detail}"
            )
            raise
        
        safe_logfire_info(
            f"Starting document upload | filename={file.filename} | content_type={file.content_type} | size={file_size} | knowledge_type={knowledge_type}"
        )
