...
class CrawlingService:
    ...
    def _check_cancellation(self):
        ...
        if self._cancelled:
            # Enhanced error context for alpha debugging
            logger.info(
                f"Crawl operation cancelled by user",
                extra={
                    "operation": "crawl_cancellation",
                    "progress_id": self.progress_id,
                    "cancelled_at": asyncio.get_event_loop().time() if hasattr(asyncio, 'get_event_loop') else None,
                    "service_id": id(self)
                }
            )
