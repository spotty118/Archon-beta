...
Handles all OpenAI embedding operations with proper rate limiting, error handling,
and Redis caching for 24-hour TTL on embedding results.

Beta Enhancement: Embeddings are cached by text hash for significant performance improvement.
"""
...
from ..cache_service import cached_operation, get_cache_service
...
async def create_embedding(text: str, provider: str | None = None, force_refresh: bool = False) -> list[float]:
    """
    ...
    Args:
        ...
        force_refresh: Skip cache and force fresh embedding generation
    ...
    try:
        ...
        # Define the actual embedding creation operation
        async def create_embedding_operation():
        ...
        # Use cached operation with 24-hour TTL for embeddings
        embedding = await cached_operation(
            "embeddings",
            text_hash,
        ...
        # Convert to appropriate exception type with enhanced context for alpha debugging
        error_msg = str(e)
        text_preview = text[:100] if text else "None"
        
        # Enhanced logging with full context for alpha mode debugging
        search_logger.error(
            f"Embedding creation failed for text: {text_preview}",
            exc_info=True,
            extra={
